	...
	public PluginInstance(File file, JarFile jarFile, Properties pluginProperties) {
		...
		if (mainClassName != null && !mainClassName.equals("")) {
			...
			Logger.error(String.format(
					"[PluginInstance] Failed to load plugin: %s. (plugin.properties->mainclass is missing)",
					file.getName()));
			return;
		}
		...
		try {
			...
			Logger.error(String.format(
					"[PluginInstance] Failed to load plugin: %s. (The class specified in plugin.properties->mainclass is missing)",
					file.getName()));
			return;
		} catch (IllegalAccessException e) {
			e.printStackTrace();
			Logger.error(
					String.format("[PluginInstance] Failed to load plugin: %s. (Check trace error)", file.getName()));
			return;
